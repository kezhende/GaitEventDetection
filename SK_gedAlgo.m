...
% figure; plot(zscore(rAcc)); hold on; plot(gaitEvent,1.5.*ones(size(gaitEvent)),'ko');
...
figure;
plot(rAcc); hold on; 
plot(vec_HS,floor(max(rAcc)).*ones(length(vec_HS),1),'go');
plot(vec_TO,floor(max(rAcc)).*ones(length(vec_TO),1),'k^');
